 ...
 */
export async function recomendarPlatillos(clienteId: number | null, limite = 4): Promise<Platillo[]> {
  const disponibles = await q<Platillo>(
  ...
  const historial = await q<{ platillo_id: number; veces_pedido: number }>(
  ...
  const categoriasFavoritas = await q<{ categoria_id: number; frecuencia: number }>(
  ...
  const populares = await recomendarPorPopularidad(limite);
...
async function recomendarPorPopularidad(limite: number): Promise<Platillo[]> {
  return q<Platillo>(
